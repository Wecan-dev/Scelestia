...
function cfw_customer_info_tab_login() {
	 ...
	 * @since 2.0.0
	 */
	$enable_enhanced_login = apply_filters( 'cfw_suppress_default_login_form', cfw_get_main()->get_settings_manager()->get_setting( 'login_style' ) !== 'woocommerce' ) && 'yes' === get_option( 'woocommerce_enable_checkout_login_reminder' );
	?>
...
function cfw_customer_info_address() {
	...
	<div id="cfw-shipping-info" class="cfw-module">
		<?php
		/**
		 * Fires at the start of customer info address module container
		 *
		 * @since 2.0.0
		 */
		do_action( sprintf( 'cfw_checkout_before_%s_address', WC()->cart->needs_shipping() ? 'shipping' : 'billing' ), WC()->checkout() );
		?>
		...
		<?php
		/**
		 * Fires after customer info address fields output
		 *
		 * @since 2.0.0
		 */
		do_action( sprintf( 'cfw_checkout_after_%s_address', WC()->cart->needs_shipping() ? 'shipping' : 'billing' ), WC()->checkout() );
		?>
	</div>
	...
	<ul class="cfw-review-pane cfw-module">
		...
		<?php if ( WC()->cart->needs_shipping() ) : ?>
			...
			<li class="shipping-method">
				...
				<div role="cell" class="col-2 cfw-review-pane-link">
					<?php if ( cfw_get_main()->get_settings_manager()->get_setting( 'skip_shipping_step' ) !== 'yes' ) : ?>
...
function cfw_order_review_step_totals_review_pane() {
	...
	<ul class="cfw-review-pane cfw-module" id="cfw-review-order-totals">
		<li>
			<div class="inner col-10 cfw-no-border">
				<div role="rowheader" class="cfw-review-pane-lab">
					<?php cfw_e( 'Subtotal', 'woocommerce' ); ?>
				</div>
			</div>

			<div role="cell" class="cfw-review-pane-content col-2 cfw-review-pane-right cfw-no-border">
			...
			<?php if ( 'itemized' === get_option( 'woocommerce_tax_total_display' ) ) : ?>
				<?php foreach ( WC()->cart->get_tax_totals() as $code => $tax ) : ?>
					<li>
						...
						<div role="cell" class="cfw-review-pane-content col-2 cfw-review-pane-right">
		...
		<li>
			...
			<div role="cell" class="cfw-review-pane-content col-2 cfw-review-pane-right cfw-order-review-total">
 ...
 * @param bool $show_cart_return_link
 */
function cfw_order_review_tab_nav( $show_cart_return_link = false ) {
	/**
	 * Fires before payment method tab navigation container
	 *
	 * @since 2.0.0
...
function cfw_coupon_module( $mobile = false ) {
	 ...
	 * @since 3.0.0
	 */
	$hide_promo_code_by_default = apply_filters( 'cfw_hide_promo_code_by_default', cfw_get_main()->get_settings_manager()->get_setting( 'enable_coupon_code_link' ) === 'yes' );
...
function cfw_form_attributes( $id = false, $row = true, $action = true ) {
	...
	?>
	id="<?php echo $id ? $id : 'checkout'; ?>" name="<?php echo $id ? $id : 'checkout'; ?>" class="cfw-customer-info-active <?php echo ( 'order_review' !== $id ) ? 'woocommerce-checkout checkout' : ''; ?> <?php echo $row ? ' row' : ''; ?>" method="POST" formnovalidate="" data-parsley-focus="first" enctype="multipart/form-data" <?php echo $action ? 'action="' . esc_url( wc_get_checkout_url() ) . '"' : ''; ?>
	<?php
		...
		<?php elseif ( $order->needs_shipping_address() ) : ?>
			<ul class="status-steps">
				...
				<?php foreach ( $order_statuses as $order_status ) : ?>
					 ...
					 * @since 3.0.0
					 *
					 * @param string $thank_you_title Thank you status icon class
					 */
...
function cfw_thank_you_map( WC_Order $order ) {
	if ( $order->needs_shipping_address() ) :
		?>
		<?php if ( cfw_get_main()->get_settings_manager()->is_premium_feature_enabled( 'enable_map_embed' ) ) : ?>
		<div id="map"></div>
		<?php endif; ?>
